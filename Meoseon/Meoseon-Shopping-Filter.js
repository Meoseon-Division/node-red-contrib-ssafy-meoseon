  ...
  function ShoppingFilter(config) {
    ...
    node.on('input', function (msg) {
      if (config.apiType === 'naver') {
        ...
        if (config.minprice)
          list = list.filter(
            (item) => Number(item.lprice) > Number(config.minprice)
          )

        if (config.maxprice)
          list = list.filter(
            (item) => Number(item.lprice) < Number(config.maxprice)
          )
        ...
        if (Array.isArray(list)) {
          if (config.minprice)
            list = list.filter(
              (item) => Number(item.SalePrice._text) > Number(config.minprice)
            )

          if (config.maxprice)
            list = list.filter(
              (item) => Number(item.SalePrice._text) < Number(config.maxprice)
            )
    ...
    node.on('input', function (msg) {
      ...
      if (config.apiType === 'naver') {
        ...
        products.forEach(function (product, idx) {
          ...
          item.productName = product.ProductName._cdata.replace(
            extractHtmlPattern,
            ''
          )
