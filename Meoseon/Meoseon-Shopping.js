    ...
    this.on('input', function (msg) {
      ...
      if (n.apiType == 'naverShopping') {
        ...
        if (node.params) {
          ...
          node.options.params = {}
          node.options.params.display = mustache.render(
            n.naverShoppingDisplay,
            new NodeContext(msg, node.context())
          )
          node.options.params.start = mustache.render(
            n.naverShoppingStart,
            new NodeContext(msg, node.context())
          )
          node.options.params.sort = n.naverShoppingSort
          node.options.params.filter = mustache.render(
            n.naverShoppingFilter,
            new NodeContext(msg, node.context())
          )
        }
        ...
        if (
          ...
          if (n.sk11stApiCode === 'ProductInfo') {
            node.params['option'] = mustache.render(
              n.sk11stOption,
              new NodeContext(msg, node.context())
            )
          }
        } else {
          node.params['pageNum'] = mustache.render(
            n.sk11stPagenum,
            new NodeContext(msg, node.context())
          )
          node.params['pageSize'] = mustache.render(
            n.sk11stPagesize,
            new NodeContext(msg, node.context())
          )
          node.params['sortCd'] = mustache.render(
            n.sk11stSortcd,
            new NodeContext(msg, node.context())
          )
          node.params['option'] = mustache.render(
          ...
          if (n.sk11stApiCode === 'ProductSearch') {
            keywords = mustache
              .render(n.sk11stKeyword, new NodeContext(msg, node.context()))
              .split(',')
            node.params['targetSearchPrd'] = mustache.render(
              n.sk11stTargetSearchPrd,
              new NodeContext(msg, node.context())
            )
          } else {
            node.params['categoryCode'] = mustache.render(
              n.sk11stCategoryCode,
              new NodeContext(msg, node.context())
            )
          }
        }
